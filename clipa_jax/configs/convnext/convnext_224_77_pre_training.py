...
def get_config(arg=None):
  ...
  arg = bvcc.parse_arg(
      arg,  res=224, runlocal=False, token_len=77, txt='bert_base', img='B/16',
  ...
  config.wandb = dict(
      ...
      experiment=f'tpu-pod-v4-64-convnext-B_16k_{arg.res}_{arg.token_len}_tok_learn_scale_lr_align_3x_ep_FLOPs',
      entity='1999ray9999'
  )
...
#   config.input.data = dict(name='liaon-400m', split='full', data_dir='')
  ...
  config.model.image_model = 'convnext'
...
#   config.total_epochs = 7.0 if not arg.runlocal else 1
  ...
  vitual_imagenet_epoch = 10000 * 3 * (20.58 / 18.36) 
...
#   config.lr = 5e-4
  ...
  warmup_steps = ((400*16384) // batch_size) # for 16k batch size  # max(int(0.03 * config.total_epochs), 100)
  config.schedule = [
