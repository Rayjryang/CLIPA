...
#   config.input.data = dict(name='liaon-400m', split='full', data_dir='')
 ...
 # dim = 768
  ...
  config.total_steps = int((12800*10000*0.2) / 2048) if not arg.runlocal else 1
  config.lr = 5e-5 * 5
  config.wd = 0.2
  warmup_steps = 250
  config.schedule = [
      #('img/.*', None),  # Freezes image tower.
      ('.*', dict(decay_type='cosine', warmup_steps=warmup_steps, min_lr=0, max_lr=5e-5 )),
  ]
  ...
  config.masked_init = ""
#   config.masked_no_load = {'dont_load': ['.*txt/pos_embedding.*','.*img/pos_embedding.*']}
  config.masked_no_load = {'dont_load': ['.*img/pos_embedding.*']}
